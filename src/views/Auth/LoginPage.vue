<template>
    <div class=" w-full flex items-center justify-center h-screen bg-gradient-to-b from-white to-emerald-200/70">
        <div class=" w-5/6 md:w-3/6 p-6 mx-2 rounded-xl bg-white bg-opacity-60 backdrop-blur-lg backdrop-filter z-30">
            <img src="/fulfil.png" alt="fulfil" class=" object-scale-down md:w-56 w-48 my-3 mx-auto">

            <form @submit.prevent="login">
                <div class=" text-left my-6">
                    <!-- <label for="email" class=" text-xs text-gray-500"> Enter Your Email</label> -->
                    <input type="email" v-model="userInfo.email" class=" w-full rounded py-2 px-3 bg-transparent border-spacing-1 border-b-2 border-emerald-800/50 
                                              drop-shadow-md text-sm" placeholder="Enter Email">
                </div>
                <div class="text-left my-6">
                    <input type="password" v-model="userInfo.password" class=" w-full rounded py-2 px-3 bg-transparent border-spacing-1 border-b-2 border-emerald-800/50 
                                              drop-shadow-md text-sm" placeholder="Enter Password">
                 </div>

                <div class=" text-center my-5">
                    <button  type="submit" v-if="! posting" class=" bg-emerald-600/80 text-gray-100 rounded-lg mx-3 px-4 py-2 drop-shadow-md 
                                                 transition transform hover:-translate-y-1 motion-reduce:transition-none motion-reduce:hover:transform-none">
                         Login 
                    </button>

                    <button type="button" v-else class="bg-indigo-300 rounded-lg mx-3 px-2 py-2" disabled>
                        <div class=" flex">
                             <p class=" mr-4 test-sm font-semibold"> Processing... </p>
                            <svg role="status" class="w-6 h-6 text-gray-200 animate-spin dark:text-gray-600 fill-emerald-600" viewBox="0 0 100 101" fill="none" xmlns="http://www.w3.org/2000/svg">
                                <path d="M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100 50.5908ZM9.08144 50.5908C9.08144 73.1895 27.4013 91.5094 50 91.5094C72.5987 91.5094 90.9186 73.1895 90.9186 50.5908C90.9186 27.9921 72.5987 9.67226 50 9.67226C27.4013 9.67226 9.08144 27.9921 9.08144 50.5908Z" fill="currentColor"/>
                                <path d="M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124 7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873 9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347 21.5619 82.5849 25.841C84.9175 28.9121 86.7997 32.2913 88.1811 35.8758C89.083 38.2158 91.5421 39.6781 93.9676 39.0409Z" fill="currentFill"/>
                            </svg>
                        </div>
                       
                    </button>

                </div>
            </form>
          
        </div>
    </div>
</template>

<script>

import axios from 'axios';
export default {
    name: 'login-page',

    data() {
        return {
              posting: false,
                userInfo:{
                    email:"",
                    password:"",
                },
        }
    },

    methods:{

        showAlert() {
                // Use sweetalert2
                this.$swal( 'Please check email and password or network');
                },

         async login(){
            this.posting = true;
                 await axios.post('login' , {
                email: this.userInfo.email,
                password: this.userInfo.password
            }).then(
                response => {
                     this.posting = false ;
                    localStorage.setItem('token', response.data.access_token);
                
                    this.$router.push('/select_sales');
                }
            ) .catch(
                error => {
                    this.showAlert();
                    this.posting = false ;
                    console.log(error);
                }
            );
         }
    }
    
}
</script>